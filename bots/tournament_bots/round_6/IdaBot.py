...
class IdaBOT(BotInterface):

    # change the name of your bot here
    def __init__(self, name="IdaBOT"):
        '''init function'''
    ...
    def handTypePlay(self, oberservation: Observation) -> Action: 
        handtype, _= getHandType(oberservation.myHand, oberservation.boardCards)
        if handtype > 8: 
            return Action.RAISE
        return Action.CALL
    ...
    def handlePreFlop(self, observation: Observation) -> Action:
        ...
        if handPercent < .50:
            return Action.RAISE
        # if my hand is top 70 percent: call
        elif handPercent < .80:
            return Action.CALL
        # else fold
        return Action.FOLD
    ...
    def handlePostFlop(self, observation: Observation) -> Action:
        ...
        opponentsMove = observation.get_opponent_history_current_stage()

        if opponentsMove:
            self.checkOpponent(observation)
    ...
    def checkOpponent(self, observation: Observation) -> Action:
        opponentsMove = observation.get_opponent_history_current_stage()
        handPercent, cards = getHandPercent(
            observation.myHand, observation.boardCards)
        if opponentsMove[-1] >= 3: 
            if handPercent <= .40:
                return Action.RAISE
        elif opponentsMove[-1] <= 3:
            return Action.FOLD
