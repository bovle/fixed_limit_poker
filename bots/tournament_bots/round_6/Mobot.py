...
class Mobot(BotInterface):
    ...
    def handlePreFlop(self, observation: Observation) -> Action:
        ...
        # 1 = small blind
        if observation.myPosition == 1 and handPercent < .55:
            return Action.CALL 
    ...
    def handleFlop(self, observation: Observation) -> Action:
        ...
        if communityHandPercent < .25 and myTwoCardHandPercent < 0.8:
            return Action.RAISE

        if handTypeValue.value < handTypeValueBoard.value-1:
            return Action.RAISE

        if handTypeValueBoard == HandType.THREEOFAKIND and myTwoCardHandPercent < 0.3:
            return Action.RAISE
    ...
    def handleTurn(self, observation: Observation) -> Action:
        ...
        if communityHandPercent < 0.2 and myTwoCardHandPercent < 0.8:
            return Action.RAISE
        ...
        if handTypeValue.value < handTypeValueBoard.value and myTwoCardHandPercent < 0.9:
            return Action.CALL
